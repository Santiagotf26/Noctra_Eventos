...
const FloatingWhatsApp = () => {
  const message = encodeURIComponent("Hola, quisiera más información sobre sus servicios de eventos.");
  const wppLink = `https://wa.me/573114971131?text=${message}`;
  ...
  return (
    <motion.a
      ...
      className="fixed bottom-6 right-6 z-50 flex items-center gap-3 px-0 w-14 md:w-auto md:px-6 h-14 rounded-full bg-[#25D366] text-white shadow-[0_0_25px_rgba(37,211,102,0.4)] hover:bg-[#20bd5a] hover:shadow-[0_0_35px_rgba(37,211,102,0.6)] transition-all group overflow-hidden justify-center"
    ...
    >
      {/* Resplandor */}
      <span className="absolute inset-0 w-full h-full rounded-full bg-white opacity-0 group-hover:opacity-20 transition-opacity" />
      ...
      <span className="hidden md:block font-bold tracking-wide text-sm pr-1 relative z-10">CONTÁCTANOS</span>
    </motion.a>
  );
};
